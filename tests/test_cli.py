...
@patch("src.api.koios.KoiosAPI.get_account_assets", return_value=[{"asset_name": "TestNFT", "quantity": 1}])
...
def test_sell_command(mock_build, mock_sign, mock_submit, monkeypatch):
    monkeypatch.setattr("src.config.Config.WALLET_ADDRESS", "addr_test1qp28mg795hwlnptmdyr47zcrc87m8kk0pwvxrwrw24ppdzzquca5pnk4ew6068z6wu4tc9ee2rr2rnn06spkkvj0llqq7fnt8u")
    runner = CliRunner()
    result = runner.invoke(cli, [
        'sell',
        '--asset-name', 'TestNFT',
